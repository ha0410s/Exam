...
<div style="width: 50%; margin-left: 25%">
	...
	<div style="margin-left:40px;margin-bottom:30px;">
		<div class="my-2 text-end px-4" style="margin-bottom: 1rem; margin-left: 500px;">
			<a href="../studentlist/PrepareStudentCreate.action">新規登録</a>
		</div>
		<form action="../studentlist/StudentList.action" method="post">
			<div style="width: 700px;
				...
				<div style="display: flex; align-items: center;">
					...
					<select class="form-select"id="select_class_num" name="f2" 
						...
						<option value="0">------</option>
						<c:if test="${teacher.id == 'admin'}">									
							<option value="131">131</option>
							<option value="201">201</option>
						</c:if>
						<c:if test="${teacher.id == 'knaka'}">
	...
	<c:otherwise>
		検索結果： ${list.size()}件<br>
		<table style="border-collapse:  collapse; width: 750px; table-layout: auto;">
			...
			<c:forEach var="student" items="${list}">
				...
				<form action="../studentlist/StudentSelect.action">
					<td style="padding-left: 3rem; padding: 1rem;">${student.ent_year}</td>
					<td style="padding-left: 3rem;"><input type="text" value="${student.no}" name="no" readonly style="border:none; text-align: center;"></td>
					<td style="padding-left: 2rem;">${student.name}</td>
					<td style="padding-left: 3rem;">${student.class_num}</td>
